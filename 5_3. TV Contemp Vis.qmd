...
#check
tvn_contemp_pred$tvmgm[[168]][["pairwise"]][["signs"]]
z <- par_ests[[168]][,,1]
```
...
tvn_plot_fnc <- function(tpSelect, par_ests, Q_shit, traitgroups, 
  ...
  for (tp in tpSelect) {
    ...
    edge_color <- ifelse(t(par_ests[, , tp]) > 0, "#0072B2", "#D55E00")
...
tvmgm_plots <- tibble(
  ...
  con_networks = lapply(seq_len(nrow(tvn_contemp_pred)), function(i) {
    tvn_plot_fnc(tpSelect, par_ests[[i]], Q_shit[[i]], traitgroups, 
                 tvmgm = tvn_contemp_pred$tvmgm[[i]], 
                 SID = tvn_contemp_pred$SID[i], 
                 bw = tvn_contemp_pred$bandwidth[i])  
  }),
  bw = tvn_contemp_pred$bandwidth
)

#test to make sure SID and network are aligned; i checked this before i commented out the title
x <- tvmgm_plots %>% filter(SID == 31624)
